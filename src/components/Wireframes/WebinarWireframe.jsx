...
// ─── Webinar Wireframe ────────────────────────────────────────────────────────
// Esqueleto P&B da página de captação de reunião/webinar. Slots editáveis quando
// `editable`. Imagens são placeholders.

const P = WEBINAR_WIREFRAME.placeholders

export default function WebinarWireframe({ content = {}, className = '', editable = false, onEdit = null }) {
  const c = { ...WEBINAR_WIREFRAME.emptyContent, ...content }
  const arr = (v, fb) => (v?.length ? v : fb)
  const E = WEBINAR_WIREFRAME.emptyContent
    ...
    <div className={`bg-white rounded-2xl overflow-hidden shadow-xl ring-1 ring-slate-200 text-slate-800 ${className}`}>
      ...
      <section className="bg-slate-900 px-6 sm:px-10 py-10">
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          ...
          {/* direita: imagem + card de métrica */}
          <div className="relative">
            <MediaBox icon="user" label="Foto" className="rounded-2xl aspect-[4/3] w-full !bg-slate-800 !border-slate-700 !text-slate-500" />
            <div className="rounded-xl border border-slate-700 bg-slate-800 p-4 mt-3">
              <Slot value={c.heroMetricLabel} placeholder={P.heroMetricLabel} path="heroMetricLabel"
                className="block text-[9px] font-semibold uppercase tracking-widest text-slate-400" />
              <Slot as="p" value={c.heroMetricValue} placeholder={P.heroMetricValue} path="heroMetricValue"
                className="block text-2xl font-extrabold text-white mt-1" />
              <Slot value={c.heroMetricSub} placeholder={P.heroMetricSub} path="heroMetricSub"
      ...
      <section className="px-6 sm:px-10 py-10 bg-slate-900 text-center">
        ...
        <div className="rounded-2xl border border-slate-700 bg-slate-800 p-5 max-w-md mx-auto mt-6 text-left">
          {['Nome completo', 'E-mail corporativo', 'WhatsApp', 'Empresa', 'Faturamento mensal'].map((f) => (
